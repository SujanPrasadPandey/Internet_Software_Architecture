function fetchLocationWeather(location, date) {
  var request = new XMLHttpRequest();
  var apiKey = "cdd1228da4be2430078ccf5f80d3d1f7"; // Replace with your OpenWeatherMap API key
  var requestUrl = "https://api.openweathermap.org/data/2.5/weather?q=" + encodeURIComponent(location) + "&dt=" + date + "&appid=" + apiKey + "&units=metric";
  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4 && request.status === 200) {
      ...
      var temperatureCelsius = response.main.temp;
      ...
      saveWeatherData(location, date, jsonData);
    }
  };
...
function saveWeatherData(location, date, weatherData) {
